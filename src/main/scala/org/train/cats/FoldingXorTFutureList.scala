...
object FoldingXorTFutureList extends FoldingList[Future] {
  ...
  override def foldTest[A](list: List[A]): XorT[Future, Exception, List[A]] = {
    val xorTList: List[XorT[Future, Exception, List[A]]] = transformToListXorT(list)
    fold(xorTList)
  }

  override def fold[A](xorTList: List[XorT[Future, Exception, List[A]]]): XorT[Future, Exception, List[A]] = {
  ...
  override def xorTFMonoid[T]: Monoid[XorT[Future, Exception, List[T]]] =
    new Monoid[XorT[Future, Exception, List[T]]] {
      ...
      override def combine(x: XorT[Future, Exception, List[T]],
                           y: XorT[Future, Exception, List[T]]): XorT[Future, Exception, List[T]] =
        for {
          v1 <- x
          v2 <- y
        } yield v1 ++ v2
  ...
  override def transformToListXorT[A](list: List[A]): List[XorT[Future, Exception, List[A]]] = {
    val xorTList = list.map(_.toFutureXorT).map(x => x.map(List(_)))
    xorTList
  }

  override def toListOfXorTOption[A](list: List[A]): List[XorT[Future, Exception, A]] = {
    list.map(_.toFutureXorT)
  }

  override def foldMap[A](xorTList: List[XorT[Future, Exception, A]]): XorT[Future, Exception, List[A]] = {
